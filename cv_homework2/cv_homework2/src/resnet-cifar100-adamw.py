...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', required=False, default=None, type=str,
                        help='CUDA device ids for GPU training (e.g., "0", "0,1", or "all" for all GPUs)')
    parser.add_argument('--multi-gpu', required=False, action='store_true',
    ...
    if options.device is None:
        ...
        if len(gpu_ids) > 1 or options.multi_gpu:
            print(f'Using multi-GPU training on devices: {gpu_ids}')
            print(f'Primary device: cuda:{gpu_ids[0]}')
            if len(gpu_ids) > 1:
                original_bsize = bsize
                bsize = bsize * len(gpu_ids)
                print(f'Batch size scaled from {original_bsize} to {bsize} for {len(gpu_ids)} GPUs')
        else:
    ...
    # AdamW optimizer: decoupled weight decay, better generalization than Adam
    # lr=0.001, betas=(0.9, 0.999), weight_decay=0.01 (higher than Adam)
    optimizer = torch.optim.AdamW(model.parameters(), lr=0.001, betas=(0.9, 0.999), weight_decay=0.01)
    print(f'Using AdamW optimizer with lr={0.001}, betas=(0.9, 0.999), weight_decay=0.01')
